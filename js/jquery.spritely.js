...
(function ($) {
  $._spritely = {
    ...
    animate: function (options) {
      ...
      if (options.type == "sprite" && options.fps) {
        if (
          ...
          $._spritely.instances[el_id]["remaining_frames"] =
            options.play_frames + 1;
        } else if (
          options.do_once &&
          !$._spritely.instances[el_id]["remaining_frames"]
        ) {
          $._spritely.instances[el_id]["remaining_frames"] =
            options.no_of_frames;
        }
        var frames;
        var animate = function (el) {
          ...
          if ($._spritely.instances[el_id]["current_frame"] == 0) {
            ...
            $._spritely.instances[el_id]["current_frame"] ==
            frames.length - 1
          ) {
          ...
          if (
            options.on_frame &&
            options.on_frame[$._spritely.instances[el_id]["current_frame"]]
          ) {
          ...
          el.css(
            "background-position",
            frames[$._spritely.instances[el_id]["current_frame"]] + "px " + yPos
          );
          if (
            options.bounce &&
            options.bounce[0] > 0 &&
        ...
        if (
          $._spritely.instances[el_id]["remaining_frames"] &&
          $._spritely.instances[el_id]["remaining_frames"] > 0
        ) {
          $._spritely.instances[el_id]["remaining_frames"]--;
        ...
        if (!$._spritely.instances[el_id]["_stopped"]) {
          ...
          var speed = options.speed || 1,
            start_x =
              $._spritely.instances[el_id]["l"] ||
              parseInt($._spritely.getBgX(el).replace("px", ""), 10) ||
              0,
            start_y =
              $._spritely.instances[el_id]["t"] ||
              parseInt($._spritely.getBgY(el).replace("px", ""), 10) ||
              0;
          ...
          if (
            ...
            switch (options.dir) {
              case "up":
              case "down":
                $._spritely.instances[el_id].remaining_frames = Math.floor(
                  (options.img_height || 0) / speed
                );
                break;
              case "left":
              case "right":
                $._spritely.instances[el_id].remaining_frames = Math.floor(
                  (options.img_width || 0) / speed
                );
                break;
            }
            $._spritely.instances[el_id].remaining_frames++;
    ...
    randomIntBetween: function (lower, higher) {
      return parseInt(
        (rand_no = Math.floor((higher - (lower - 1)) * Math.random()) + lower)
      );
    },
    getBgUseXY: (function () {
      try {
    ...
    _spStrip: function (s, chars) {
      ...
      while (s.length) {
        ...
        for (i = 0; i < chars.length; i++) {
          var ss = s.slice(0, 1);
          sr = s.slice(1);
          if (chars.indexOf(ss) > -1) s = sr;
          else nos = true;
        }
        for (i = 0; i < chars.length; i++) {
          var se = s.slice(-1);
          sr = s.slice(0, -1);
          if (chars.indexOf(se) > -1) s = sr;
          else noe = true;
        }
        if (nos && noe) return s;
      }
  ...
  $.fn.extend({
    spritely: function (options) {
      var $this = $(this),
        ...
        background_image_src = $._spritely._spStrip(
    ...
    spStop: function (bool) {
      this.each(function () {
        ...
        if ($._spritely.instances[el_id]["options"]["fps"]) {
          $._spritely.instances[el_id]["_last_fps"] =
    ...
    lockTo: function (el, options) {
      $(this).each(function () {
        ...
        $._spritely.instances[el_id]["interval"] = window.setInterval(
          function () {
            if ($._spritely.instances[el_id]["lock_to"]) {
              var locked_el = $._spritely.instances[el_id]["locked_el"];
              var locked_to_el = $._spritely.instances[el_id]["lock_to"];
              var locked_to_options =
                $._spritely.instances[el_id]["lock_to_options"];
              var locked_to_el_w = locked_to_options.bg_img_width;
              var locked_to_el_h = locked_to_el.height();
              var locked_to_el_y = $._spritely.getBgY(locked_to_el);
              var locked_to_el_x = $._spritely.getBgX(locked_to_el);
              var el_l =
                parseInt(locked_to_el_x) + parseInt(locked_to_options["left"]);
              var el_t =
                parseInt(locked_to_el_y) + parseInt(locked_to_options["top"]);
    ...
    destroy: function () {
      ...
      if (
        $._spritely.instances[el_id] &&
        $._spritely.instances[el_id]["interval"]
      ) {
